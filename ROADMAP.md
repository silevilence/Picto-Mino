...
- [ ] 关卡调整
    - [ ] 关卡信息保存为统一的 `.level` 文件
        - [ ] `.level` 文件内容为相应的关卡文件zip压缩后改后缀
        - [ ] 内部内容如下：
            - [ ] `level.json`，关卡数据
            - [ ] `*.shape.json`，每个`.shape.json`定义一个自定义形状
            - [ ] `metadata.json`，元数据文件，指定如版本信息（目前这个为版本1，为以后格式破坏性升级兼容用）、颜色索引、shape的索引（索引内置形状和自定义形状）等信息，以后有什么要新增的元数据也加这里
    - [ ] 内置关卡保存在内置资源中 `res://Levels`
        - [ ] `res://Levels/index.json` 为关卡索引，按顺序在Levels下取相应的 `.level` 关卡文件
    - [ ] 内置形状
        - [ ] `res://Shapes` 下放一些内置的形状，每个形状为一个 `.shape.json` 文件，为 `ShapeData` 序列化保存
